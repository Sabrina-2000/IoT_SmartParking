...
while True:
    ...
    publish.single(topic="v1/devices/me/telemetry", payload='{"value":' + str(available) +'}', hostname="thingsboard.cloud", auth = {'username':"GuIKhZkC23Vax6lXweiC", 'password':""})
    publish.single(topic="v1/devices/me/telemetry", payload='{"value":' + str(count) +'}', hostname="thingsboard.cloud", auth = {'username':"bPtWLBI9knv9dr95T1TG", 'password':""})
    ...
    if(int(action) == 1):
        ...
        if(slots[0] == plate):
            ...
            available += 1
            count -= 1
            publish.single(topic="v1/devices/me/telemetry", payload='{"value":' + str(slots[0]) +'}', hostname="thingsboard.cloud", auth = {'username':"8tEX7BYJYpXQH1Sb4qxFq", 'password':""})
                
        elif(slots[1] == plate):
            slots[1] = "-"
            available += 1
            count -= 1
            publish.single(topic="v1/devices/me/telemetry", payload='{"value":' + str(slots[1]) +'}', hostname="thingsboard.cloud", auth = {'username':"1ngjHTxUSiI4Om1pD3hW", 'password':""})
             
        elif(slots[2] == plate):
            slots[2] = "-"
            available += 1
            count -= 1
            publish.single(topic="v1/devices/me/telemetry", payload='{"value":' + str(slots[2]) +'}', hostname="thingsboard.cloud", auth = {'username':"igrPUeCoCNRaxenzIfyn", 'password':""})
		...
		publish.single(topic="v1/devices/me/telemetry", payload='{"value":' + str(plate) +'}', hostname="thingsboard.cloud", auth = {'username':"1ngjHTxUSiI4Om1pD3hW", 'password':""}) 
            ...
            if(num == 0):
                slots[0] = plate
                display.lcd_display_string("Slot:A1", 1)
                publish.single(topic="v1/devices/me/telemetry", payload='{"value":' + str(plate) +'}', hostname="thingsboard.cloud", auth = {'username':"8tEX7BYJYpXQH1Sb4qxF", 'password':""})
            else:
                slots[1] = plate
                display.lcd_display_string("Slot:A2", 1)
                publish.single(topic="v1/devices/me/telemetry", payload='{"value":' + str(plate) +'}', hostname="thingsboard.cloud", auth = {'username':"1ngjHTxUSiI4Om1pD3hW", 'password':""})
